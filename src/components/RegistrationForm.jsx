...
import { CheckCircle, AlertCircle, Loader, DollarSign, Clock, Award, Briefcase, Target, Zap } from 'lucide-react';
import {
  DOCUMENT_TYPES,
  GENDERS,
  STUDY_SCHEDULES,
...
const RegistrationForm = () => {
  ...
  // Form data from backend (will use constants as fallback)
  const [formData, setFormData] = useState({
    ...
    socioeconomicStrata: SOCIOECONOMIC_STRATA,
    programmingLevels: PROGRAMMING_LEVELS,
  ...
  useEffect(() => {
    const loadFormData = async () => {
      try {
        ...
        // {
        //   documentTypes: [{value: 'cc', label: 'Cédula de Ciudadanía'}, ...],
        //   genders: [{value: 'masculino', label: 'Masculino'}, ...],
        //   ... etc
        // }
        setFormData({
          documentTypes: data.documentTypes || DOCUMENT_TYPES,
          genders: data.genders || GENDERS,
          studySchedules: data.studySchedules || STUDY_SCHEDULES,
          occupations: data.occupations || OCCUPATIONS,
          educationLevels: data.educationLevels || EDUCATION_LEVELS,
          departments: data.departments || DEPARTMENTS,
          municipalities: data.municipalities || MUNICIPALITIES,
          socioeconomicStrata: data.socioeconomicStrata || SOCIOECONOMIC_STRATA,
          programmingLevels: data.programmingLevels || PROGRAMMING_LEVELS,
          agreementTypes: data.agreementTypes || AGREEMENT_TYPES,
        });
      } catch (error) {
        console.error('Error al cargar datos del formulario:', error);
        // Si falla, usar datos por defecto (constantes)
        // Ya están cargados en el estado inicial
      } finally {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-start">
          ...
          <div className="space-y-6">
            ...
            {/* ISA Model Box */}
            <div className="gradient-bg rounded-2xl p-6 space-y-4">
              <h3 className="text-xl font-bold text-white">Modelo ISA - Sin riesgo</h3>
              <ul className="space-y-3">
                <li className="flex items-start gap-2 text-white">
                  <CheckCircle className="w-5 h-5 flex-shrink-0 mt-0.5" />
                  <span className="text-sm">$0 de inversión inicial</span>
                </li>
                <li className="flex items-start gap-2 text-white">
                  <CheckCircle className="w-5 h-5 flex-shrink-0 mt-0.5" />
                  <span className="text-sm">Solo pagas cuando ganes más de $2M</span>
                </li>
                <li className="flex items-start gap-2 text-white">
                  <CheckCircle className="w-5 h-5 flex-shrink-0 mt-0.5" />
                  <span className="text-sm">Acompañamiento hasta conseguir empleo</span>
                </li>
                <li className="flex items-start gap-2 text-white">
                  <CheckCircle className="w-5 h-5 flex-shrink-0 mt-0.5" />
                  <span className="text-sm">Preparación para entrevistas técnicas</span>
                </li>
              </ul>
            </div>
          </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  ...
                  >
                    ...
                    {formData.studySchedules.map((schedule) => (
              ...
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  ...
                  >
                    ...
                    {selectedDepartment &&
                      formData.municipalities[selectedDepartment]?.map((city) => (
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  >
                    ...
                    {formData.socioeconomicStrata.map((strata) => (
                ...
                <div className="space-y-1">
                  ...
                  >
                    ...
                    {formData.programmingLevels.map((level) => (
                      <option key={level.value} value={level.value}>
                        {level.label}
                      </option>
                    ))}
              ...
              <div className="space-y-1">
                ...
                >
                  ...
                  {formData.agreementTypes.map((type) => (
                    <option key={type.value} value={type.value}>
                      {type.label}
                    </option>
                  ))}
              ...
              {submitStatus && (
                <div
                  className={`p-3 rounded-lg flex items-center gap-2 text-sm ${submitStatus.type === 'success'
